...
static TaskNode_t* AppInfoToTaskNode(const AppInfo* appInfo)
{
    ...
    SetDescValue(AddrOffset(ret->task.ldt, LDT_Code32Index), 0u, PGEIndex((uint32_t)(1u << 32)) - 1u, DA_32 + DA_C + DA_LIMIT_4K + DA_DPL3);
    SetDescValue(AddrOffset(ret->task.ldt, LDT_Data32Index), 0u, PGEIndex((uint32_t)(1u << 32)) - 1u, DA_32 + DA_DRW + DA_LIMIT_4K + DA_DPL3);
